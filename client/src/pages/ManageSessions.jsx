import { PaperClipIcon } from "@heroicons/react/20/solid";

export default function ManageSessions() {
  return (
    <>
      <div>
        <div className="px-4 sm:px-">
          <h3 className="text-base font-bold leading-7 text-red-700">
            Session Details
          </h3>
        </div>
        <div className="px-2 py-2 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-1">
          <dt className="text-sm font-bold leading-6 text-gray-900">Date</dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            2024-03-30
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-1">
          <dt className="text-sm font-bold leading-6 text-gray-900">
            Location
          </dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            Home
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-1">
          <dt className="text-sm font-bold leading-6 text-gray-900">Results</dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            -500
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">
            Hours Played
          </dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            3
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">Notes</dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            Played Texas Hold'em with friends. Had a good run with some lucky
            hands.
          </dd>
        </div>
      </div>

      <div>
        <div className="px-4 sm:px-">
          <h3 className="text-base font-bold leading-7 text-red-700">
            Session Details
          </h3>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">Date</dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            2024-03-28
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">
            Location
          </dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            The Golden Casino
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">Results</dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            300
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">
            Hours Played
          </dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            4
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">Notes</dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            Participated in a tournament. Managed to reach the final table and
            secure a decent profit.
          </dd>
        </div>
      </div>

      <div>
        <div className="px-4 sm:px-">
          <h3 className="text-base font-bold leading-7 text-red-700">
            Session Details
          </h3>
        </div>
            <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
              <dt className="text-sm font-bold leading-6 text-gray-900">
                Date
              </dt>
              <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
                2024-03-25
              </dd>
            </div>
            <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
              <dt className="text-sm font-bold leading-6 text-gray-900">
                Location
              </dt>
              <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
                Online
              </dd>
            </div>
            <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
              <dt className="text-sm font-bold leading-6 text-gray-900">
                Results
              </dt>
              <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
                -150
              </dd>
            </div>
            <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
              <dt className="text-sm font-bold leading-6 text-gray-900">
                Hours Played
              </dt>
              <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
                5
              </dd>
            </div>
            <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
              <dt className="text-sm font-bold leading-6 text-gray-900">
                Notes
              </dt>
              <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
                Experimented with a new strategy but ended up losing. Need to
                reassess my approach.
              </dd>
        </div>
      </div>

      <div>
        <div className="px-4 sm:px-">
          <h3 className="text-base font-bold leading-7 text-red-700">
            Session Details
          </h3>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">Date</dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            2024-03-30
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">
            Location
          </dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            The Mirage
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">Results</dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            200
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">
            Hours Played
          </dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            3.5
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">Notes</dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            Joined a friendly poker table at The Mirage. Managed to read
            opponents well and hit some crucial hands, leading to a profitable
            session.
          </dd>
        </div>
      </div>

      <div>
        <div className="px-4 sm:px-">
          <h3 className="text-base font-bold leading-7 text-red-700">
            Session Details
          </h3>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">Date</dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            2024-03-30
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">
            Location
          </dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            Bellagio
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">Results</dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            400
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">
            Hours Played
          </dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            5
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">Notes</dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            Participated in a Sit-and-Go tournament at Bellagio. Made some
            aggressive moves early on and managed to secure first place.
          </dd>
        </div>
      </div>

      <div>
        <div className="px-4 sm:px-">
          <h3 className="text-base font-bold leading-7 text-red-700">
            Session Details
          </h3>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">Date</dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            2024-03-30
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">
            Location
          </dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            Home
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">Results</dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            100
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">
            Hours Played
          </dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            2
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">Notes</dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            Hosted a poker night with friends. Had a great time playing Texas
            Hold'em and won some pots with well-timed bluffs.
          </dd>
        </div>
      </div>

      <div>
        <div className="px-4 sm:px-">
          <h3 className="text-base font-bold leading-7 text-red-700">
            Session Details
          </h3>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">Date</dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            2024-03-30
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">
            Location
          </dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            Lago
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">Results</dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            900
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">
            Hours Played
          </dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            5
          </dd>
        </div>
        <div className="px-4 py-6 sm:grid sm:grid-cols-12 sm:gap-4 sm:px-0">
          <dt className="text-sm font-bold leading-6 text-gray-900">Notes</dt>
          <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
            Hosted a poker night with friends
          </dd>
        </div>
      </div>
    </>
  );
}
